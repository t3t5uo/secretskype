      ...
      <div className="container mx-auto px-4 py-6">
        ...
        >
          ...
          <div className="flex flex-col space-y-2 md:space-y-0 md:space-x-2 md:flex-row">
            ...
            <button 
              type="submit"
              className="p-2 bg-blue-400 text-white rounded w-full md:w-auto hover:bg-blue-500"
            >
              Filter
            </button>
          </div>
        ...
        {models.map(model => (
          <Link key={model.id} href={`/model/${model.fields.slug}`}>
            <div className="cursor-pointer border rounded-s overflow-hidden hover:shadow-xl transition-shadow duration-300">
              <img src={model.fields.thumb_image[0].url} alt={model.fields.name} className="w-full h-48 object-cover" />
              <div className="p-2 flex justify-between items-center">
                <h2 className="text-lg truncate">{model.fields.name}</h2>
                {model.fields.price ? (
